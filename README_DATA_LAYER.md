...
**`/dashboard` trend strip:** **`useDashboardData`** is invoked early so **`activeProgram.startDate`** can define preset **from–to** bounds (**forward** `N` days for 1–4 wk chips). Workouts in that window use **`useWorkouts`** with the inclusive range. The page also composes **`useNutritionDay`**, **`useRecoveryEntry`**, and **`useSupplementLog`** for **`selectedTrendDate`** so browsing dates updates schedule and cards with the **same per-day query keys** as domain pages. **`useUpdateProgram`** patches **`Program.startDate`** (Week 1 start) and participates in the same invalidation story as other training writes.
