...
export function StepQuestionnaire({ onNext, onBack }: StepQuestionnaireProps) {
  ...
  function setInjury(idx: number, field: keyof QuestionnaireAnswers['injuries'][0], value: string) {
    setAnswers((prev) => {
    ...
    <div className="flex flex-col gap-7 py-4">
      ...
      <div>
        ...
        <p className="mt-2 text-sm text-[color:var(--text-1)]">
          ...
          <code className="text-[color:var(--accent-light)] text-xs bg-[rgba(16,16,16,0.78)] border border-[color:color-mix(in_srgb,var(--accent)_25%,transparent)] px-1.5 py-0.5 rounded">
            null
          </code>{' '}
            ...
            <div className="px-5 pb-5 flex flex-col gap-4 border-t border-[color:color-mix(in_srgb,var(--accent)_22%,transparent)]">
              <div className="pt-4 flex flex-col gap-2 text-sm text-[color:var(--text-1)] leading-relaxed">
                <p>
                  ...
                  IRONMIND right away. You can replace it with your own personalised data at any
                  time from{' '}
                ...
                <p className="font-semibold text-[color:var(--text-0)] mt-1">
                  When you&apos;re ready to go custom, here&apos;s the full process:
                </p>
                ...
                <p className="text-xs text-[color:var(--text-2)] mt-1">
                  Need a refresher anytime? The{' '}
                  <strong className="text-[color:var(--text-0)]">User Guide</strong> in the app
      ...
      >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <Field label="Body Fat % Estimate">
            <input
              ...
              min={3}
              max={60}
              value={answers.bodyFatEstimate}
              onChange={(e) => set('bodyFatEstimate', e.target.value)}
              placeholder="e.g. 18"
            />
          </Field>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Field label="Years Training">
            <input
              ...
              type="number"
              min={0}
              value={answers.yearsTraining}
              onChange={(e) => set('yearsTraining', e.target.value)}
              placeholder="e.g. 8"
            />
          </Field>
        ...
        <Field label="Training History (brief summary)">
          <textarea
            ...
            placeholder="What splits / programs have you run? Any phases of enhanced performance?"
          />
        </Field>
        <Field label="Competition History">
          <textarea
            className={inputCls}
            rows={2}
            value={answers.competitionHistory}
            onChange={(e) => set('competitionHistory', e.target.value)}
            placeholder="Shows entered, placing, division, natural or not"
          />
        </Field>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <Field label="Timeline">
            <input
              ...
              placeholder="e.g. 16 weeks, show on Oct 12"
            />
          </Field>
        </div>
        <Field label="Primary Goal (one sentence)">
          <textarea
            ...
            placeholder="e.g. Compete at 90kg classic physique on Oct 12 in conditioned shape"
          />
        </Field>
        <Field label="What does success look like?">
          <textarea
            ...
            placeholder="Describe the physique, performance, or lifestyle outcome you want"
          />
        </Field>
        ...
        {(
          ...
          <Field key={k} label={`Secondary Goal ${i + 1}`}>
            <input
              ...
              placeholder="e.g. Improve shoulder width, hit 140kg bench, sleep 8hrs consistently"
            />
          </Field>
        ))}
        ...
        {([0, 1, 2] as const).map((idx) => (
          <div
            key={idx}
            className="p-4 rounded-xl border border-[rgba(65,50,50,0.30)] bg-[rgba(8,8,8,0.5)] flex flex-col gap-3"
          >
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[color:var(--text-1)]">
              Injury {idx + 1}
            </p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <Field label="Injury Name">
                <input
                  className={inputCls}
                  value={answers.injuries[idx].name}
                  onChange={(e) => setInjury(idx, 'name', e.target.value)}
                  placeholder="e.g. Left shoulder labrum"
                />
              </Field>
              <Field label="Current Status">
                <input
                  className={inputCls}
                  value={answers.injuries[idx].status}
                  onChange={(e) => setInjury(idx, 'status', e.target.value)}
                  placeholder="e.g. Managed, no overhead press"
                />
              </Field>
              <Field label="Movements to Avoid">
                <input
                  className={inputCls}
                  value={answers.injuries[idx].avoid}
                  onChange={(e) => setInjury(idx, 'avoid', e.target.value)}
                  placeholder="e.g. Overhead press, upright rows"
                />
              </Field>
              <Field label="Safe Substitutions">
                <input
                  className={inputCls}
                  value={answers.injuries[idx].substitute}
                  onChange={(e) => setInjury(idx, 'substitute', e.target.value)}
                  placeholder="e.g. Lateral raises, cable side delts"
                />
              </Field>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Field label="Medical Conditions">
            <input
              ...
              placeholder="e.g. Hypertension — monitored"
            />
          </Field>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {(['strongPart1', 'strongPart2', 'strongPart3'] as const).map((k, i) => (
            <Field key={k} label={`Strong Body Part ${i + 1}`}>
              <input
                ...
                value={answers[k]}
                onChange={(e) => set(k, e.target.value)}
                placeholder="e.g. Back, Arms, Hamstrings"
              />
            </Field>
          ))}
          {(['weakPart1', 'weakPart2', 'weakPart3'] as const).map((k, i) => (
            <Field key={k} label={`Weak Body Part ${i + 1}`}>
              <input
                ...
                value={answers[k]}
                onChange={(e) => set(k, e.target.value)}
                placeholder="e.g. Chest, Calves, Front delts"
              />
            </Field>
          ))}
          <Field label="Symmetry Score (1–10)">
            <input
              ...
              min={1}
              max={10}
              value={answers.symmetryScore}
              onChange={(e) => set('symmetryScore', e.target.value)}
              placeholder="e.g. 7"
            />
          </Field>
        ...
        <Field label="Equipment Available">
          <textarea
            ...
            placeholder="e.g. Barbells, dumbbells 5–50kg, cable machine, smith machine, leg press"
          />
        </Field>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {(
            ...
            ] as [keyof QuestionnaireAnswers, string][]
          ).map(([k, label]) => (
          ...
          <Field label="Other Key Lift">
            <input
              className={inputCls}
              value={answers.otherLift}
              onChange={(e) => set('otherLift', e.target.value)}
              placeholder="e.g. Cable row 3×10 @ 70kg"
            />
          </Field>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Field label="Current Cardio Type">
            <input
              className={inputCls}
              value={answers.currentCardioType}
              onChange={(e) => set('currentCardioType', e.target.value)}
              placeholder="e.g. Incline treadmill, assault bike"
            />
          </Field>
          <Field label="Sessions / Week">
            <input
              ...
              type="number"
              min={0}
              value={answers.currentCardioFrequency}
              onChange={(e) => set('currentCardioFrequency', e.target.value)}
              placeholder="e.g. 3"
            />
          </Field>
          <Field label="Duration (minutes)">
            <input
              className={inputCls}
              type="number"
              value={answers.currentCardioDuration}
              onChange={(e) => set('currentCardioDuration', e.target.value)}
              placeholder="e.g. 30"
            />
          </Field>
          ...
          <Field label="Cardio Goal">
            <input
              className={inputCls}
              value={answers.cardioGoal}
              onChange={(e) => set('cardioGoal', e.target.value)}
              placeholder="e.g. Improve conditioning, accelerate fat loss"
            />
          </Field>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <Field label="Adherence Rating (1–10)">
            <input
              ...
              min={1}
              max={10}
              value={answers.adherenceRating}
              onChange={(e) => set('adherenceRating', e.target.value)}
              placeholder="e.g. 8"
            />
          </Field>
        </div>
        <Field label="Meal Timing Preference">
          <input
            className={inputCls}
            value={answers.mealTimingPreference}
            onChange={(e) => set('mealTimingPreference', e.target.value)}
            placeholder="e.g. Largest meals post-workout, no eating past 9pm"
          />
        </Field>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <Field label="Digestive Issues">
            <input
              ...
              onChange={(e) => set('digestiveIssues', e.target.value)}
              placeholder="e.g. IBS, bloating from high-fibre days"
            />
          </Field>
        </div>
        ...
        {(
          ...
          <Field key={k} label={label}>
            <textarea
              ...
              value={answers[k] as string}
              onChange={(e) => set(k, e.target.value)}
              placeholder="e.g. Creatine 5g, Vitamin D3 5000IU, Omega-3 3g"
            />
          </Field>
        ))}
        <Field label="Supplement Budget">
          <input
            className={inputCls}
            value={answers.supplementBudget}
            onChange={(e) => set('supplementBudget', e.target.value)}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Field label="Average Sleep (hours)">
            <input
              ...
              max={12}
              step={0.5}
              value={answers.avgSleepHours}
              onChange={(e) => set('avgSleepHours', e.target.value)}
              placeholder="e.g. 7"
            />
          </Field>
          ...
          <Field label="Occupation / Activity Level">
            <input
              ...
              placeholder="e.g. Software engineer — sedentary office work"
            />
          </Field>
          <Field label="Daily Steps (average)">
            <input
              ...
              onChange={(e) => set('dailySteps', e.target.value)}
          ...
          <Field label="Recovery Score / HRV Note">
            <input
              className={inputCls}
              value={answers.recoveryScore}
              onChange={(e) => set('recoveryScore', e.target.value)}
              placeholder="e.g. Garmin Body Battery avg 60"
            />
          </Field>
          <Field label="Alcohol (units / week)">
            <input
              className={inputCls}
              value={answers.alcoholPerWeek}
              onChange={(e) => set('alcoholPerWeek', e.target.value)}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <Field label="Primary Communication Channel">
            <input
              className={inputCls}
              value={answers.primaryCommChannel}
              onChange={(e) => set('primaryCommChannel', e.target.value)}
              placeholder="e.g. ChatGPT, Claude, WhatsApp with coach"
            />
          </Field>
        </div>
        <Field label="Biggest Challenge Right Now">
          <textarea
            ...
            placeholder="e.g. Consistency with nutrition on work travel days"
          />
        </Field>
        <Field label="Anything Else the Coach Should Know">
          <textarea
            ...
            placeholder="Family commitments, surgery scheduled, upcoming vacation, recent bloodwork results, past PED use, etc."
          />
        </Field>
      </div>
