...
export default function WorkoutHistoryPage() {
  ...
  const { data: workouts, isLoading } = useRecentWorkouts(userId, 28);
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <TrainingMediaModal
        open={mediaOpen}
        sessionTitle={todaySession?.name}
              ...
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="text-xs text-[color:var(--text-2)]">
                    {formatDisplayDate(w.date)} · Day {w.cycleDayNumber}
                  </p>
                </div>
                <span className="font-mono tabular-nums text-sm text-[color:var(--text-2)]">
              ...
              {w.exercises.length > 0 && (
                <div className="flex flex-wrap gap-1.5">
                  {w.exercises.slice(0, 4).map((e) => (
                    <span
                      key={e.exerciseId}
                      className="text-xs px-2 py-0.5 rounded-md bg-[rgba(18,14,14,0.6)] text-[color:var(--text-2)]"
                    >
                      {e.name}
                    </span>
                  ))}
                  {w.exercises.length > 4 && (
                    <span className="text-xs px-2 py-0.5 rounded-md bg-[rgba(18,14,14,0.6)] text-[color:var(--text-2)]">
                      +{w.exercises.length - 4}
                    </span>
                  )}
                </div>
