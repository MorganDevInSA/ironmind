...
export default function PhysiquePage() {
  ...
  const [form, setForm] = useState({ bodyweight: '', waist: '', chest: '', hips: '', leftArm: '', rightArm: '', leftThigh: '', rightThigh: '', coachNotes: '' });
  ...
  const hasAnyMeasurements = measurementData.some(d =>
    measurementKeys.some(k => d[k as keyof typeof d] !== undefined)
  );

  const lastCheckIn = recentCheckIns?.[0];
  const prevCheckIn = recentCheckIns?.[1];
  const weightDelta = lastCheckIn && prevCheckIn ? lastCheckIn.bodyweight - prevCheckIn.bodyweight : null;
        ...
        <div className="glass-panel p-5 space-y-5">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <Field label="Hips" value={form.hips} onChange={v => setForm(f => ({ ...f, hips: v }))} unit="cm" />
              <Field label="L Arm" value={form.leftArm} onChange={v => setForm(f => ({ ...f, leftArm: v }))} unit="cm" />
              <Field label="R Arm" value={form.rightArm} onChange={v => setForm(f => ({ ...f, rightArm: v }))} unit="cm" />
              <Field label="L Thigh" value={form.leftThigh} onChange={v => setForm(f => ({ ...f, leftThigh: v }))} unit="cm" />
            </div>
          </div>
            ...
            <LineChart data={measurementData} margin={{ top: 4, right: 4, bottom: 0, left: -20 }}>
              ...
              {measurementKeys.map(key => (
                <Line key={key} type="monotone" dataKey={key}
                  stroke={measurementColors[key]} strokeWidth={2}
                  dot={{ r: 3, fill: measurementColors[key], strokeWidth: 0 }}
