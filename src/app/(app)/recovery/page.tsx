    ...
    <div className="bg-[rgba(14,11,11,0.95)] border border-[rgba(65,50,50,0.3)] rounded-xl px-3 py-2 shadow-xl">
      ...
      {payload.map(p => (
        <div key={p.name} className="flex items-center gap-2 text-sm">
          ...
          <span className="font-mono tabular-nums text-[color:var(--text-0)] font-semibold">{p.value}</span>
        </div>
      ))}
    </div>
  );
  ...
  return (
    <div className="space-y-2">
      <div className="flex justify-between items-center">
        ...
        <span className="font-mono tabular-nums text-[color:var(--text-0)] font-semibold text-sm">{value}{unit}</span>
      </div>
...
export default function RecoveryPage() {
  ...
  const [form, setForm] = useState({ sleepHours: 7.5, sleepQuality: 7, hrv: 55, stress: 3, energy: 7, doms: 4, mood: 3 });
  ...
  const handleSubmit = () => {
    ...
    saveRecovery({
      date: todayStr,
      entry: { date: todayStr, sleepHours: form.sleepHours, sleepQuality: form.sleepQuality, hrv: form.hrv, stress: form.stress, energy: form.energy, doms: form.doms, mood: form.mood, readinessScore },
    });
  };
  ...
  const statusCard = latestEntry && (
    <div className={cn('glass-panel p-4 flex items-center gap-4',
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="font-semibold text-[color:var(--text-0)] truncate">
          {latestEntry.date === today() ? 'Today already logged' : formatDisplayDate(latestEntry.date)}
        </p>
          ...
          <div className="glass-panel p-5 space-y-6">
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                ...
                <span className="font-mono tabular-nums text-[color:var(--text-0)] font-semibold text-sm">{form.sleepHours}h</span>
              </div>
              <input type="range" min={3} max={12} step={0.5} value={form.sleepHours}
                onChange={e => setForm(f => ({ ...f, sleepHours: Number(e.target.value) }))}
                className="w-full h-2 rounded-full" style={{ accentColor: 'var(--accent)' }} />
              <div className="flex justify-between text-xs text-[color:var(--text-2)]"><span>3h</span><span>12h</span></div>
            </div>
                ...
                {MOODS.map((emoji, i) => (
                  <button key={i} onClick={() => setForm(f => ({ ...f, mood: i + 1 }))}
                    className={cn('flex-1 py-2 rounded-lg text-xl border transition-all',
                      form.mood === i + 1 ? 'border-[color:color-mix(in_srgb,var(--accent)_50%,transparent)] bg-[rgba(16,16,16,0.78)]' : 'border-[rgba(65,50,50,0.25)]')}>
                    {emoji}
                  </button>
                ))}
              </div>
        ...
        <div className="space-y-5">
          {chartData.length === 0 ? (
            <div className="glass-panel p-8 text-center">
              <p className="text-[color:var(--text-2)]">No recovery history yet. Log your first entry to see trends.</p>
            </div>
          ) : (
            <>
              {statusCard}
              ...
              <div className="glass-panel p-5">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <span className="ml-auto text-xs text-[color:var(--text-2)]">Last {chartData.length} days</span>
                </div>
                <ResponsiveContainer width="100%" height={200}>
                  <AreaChart data={chartData} margin={{ top: 4, right: 4, bottom: 0, left: -20 }}>
                    ...
                    <XAxis dataKey="date" tick={{ fontSize: 10, fill: 'var(--text-2)' }} tickLine={false} axisLine={false} />
                    <YAxis domain={[0, 100]} tick={{ fontSize: 10, fill: 'var(--text-2)' }} tickLine={false} axisLine={false} />
              ...
              <div className="glass-panel p-5">
                ...
                <ResponsiveContainer width="100%" height={200}>
                  <LineChart data={chartData} margin={{ top: 4, right: 4, bottom: 0, left: -20 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="rgba(65,50,50,0.12)" />
                    <XAxis dataKey="date" tick={{ fontSize: 10, fill: 'var(--text-2)' }} tickLine={false} axisLine={false} />
                    <YAxis yAxisId="sleep" domain={[0, 12]} tick={{ fontSize: 10, fill: 'var(--text-2)' }} tickLine={false} axisLine={false} />
                    <YAxis yAxisId="hrv" orientation="right" domain={[0, 120]} tick={{ fontSize: 10, fill: 'var(--text-2)' }} tickLine={false} axisLine={false} />
                    <Tooltip content={<ChartTooltip />} />
              ...
              <div className="glass-panel p-5">
                <h3 className="font-semibold text-[color:var(--text-0)] mb-4">Energy · Stress · DOMS</h3>
                <ResponsiveContainer width="100%" height={200}>
                  <LineChart data={chartData} margin={{ top: 4, right: 4, bottom: 0, left: -20 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="rgba(65,50,50,0.12)" />
                    <XAxis dataKey="date" tick={{ fontSize: 10, fill: 'var(--text-2)' }} tickLine={false} axisLine={false} />
                    <YAxis domain={[0, 10]} tick={{ fontSize: 10, fill: 'var(--text-2)' }} tickLine={false} axisLine={false} />
                    <Tooltip content={<ChartTooltip />} />
                    <Legend iconType="circle" wrapperStyle={{ fontSize: '11px' }} />
              ...
              {radarData.length > 0 && (
                <div className="glass-panel p-5">
                  <h3 className="font-semibold text-[color:var(--text-0)] mb-1">Recovery Profile</h3>
                  <p className="text-xs text-[color:var(--text-2)] mb-3">{latestEntry && formatDisplayDate(latestEntry.date)}</p>
                  <ResponsiveContainer width="100%" height={280}>
                    <RadarChart data={radarData} margin={{ top: 10, right: 30, bottom: 10, left: 30 }}>
                      <PolarGrid stroke="rgba(65,50,50,0.2)" />
