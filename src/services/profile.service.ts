import { CURRENT_DATA_SCHEMA_VERSION, type AthleteProfile, type UserData } from '@/lib/types';
...
export async function getUserData(userId: string): Promise<UserData | null> {
  return withService('profile', 'read user data', () =>
    getDocument<UserData>(collections.users, userId, userConverter),
  );
}
...
export async function updateUserData(userId: string, data: Partial<UserData>): Promise<void> {
  return withService('profile', 'update user data', () =>
    setDocument<UserData>(collections.users, userId, data as UserData, userConverter),
  );
}
