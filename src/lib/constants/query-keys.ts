...
export function queryKeys(userId: string) {
  return {
    ...
    physique: {
      ...
      checkIns: (limit = 500) => [userId, 'physique', 'check-ins', limit] as const,
    ...
    dashboard: {
      all: [userId, 'dashboard'] as const,
      bundle: (calendarDate: string) => [userId, 'dashboard', 'bundle', calendarDate] as const,
    },
  };
}
