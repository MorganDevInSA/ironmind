...
- **Components:** **`IronmindLogo`**: **sidebar** + **top bar** use **`logoCombined`** (same asset as **`/login`**, including narrow viewports); **register** uses male vs female by theme (hot-pink → female). **`/login`** uses the combined mark directly via **`next/image`** (not required to use **`IronmindLogo`**). Combined raster is **1536×1024**; sizing and responsive **`sizes`** live in **`ironmind-logo.tsx`** (do not scatter Tailwind on **`sidebar.tsx`** / **`top-bar.tsx`**).
