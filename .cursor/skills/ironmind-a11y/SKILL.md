...
- **Logo:** **`IronmindLogo`** uses **`alt="IRONMIND"`** on the image; navigation semantics stay on **`Link`** / buttons — the mark is branding only. Sidebar + **top bar** use **`logoCombined`**; **`auth`** uses theme shields — see **`ironmind-visual-persona`** / **`LOGO-BRIEF`**.
