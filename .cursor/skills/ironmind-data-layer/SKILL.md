...
**`/dashboard` and the trend day strip:** The page uses the bundle for **profile**, **active program**, **weekly volume**, and loading gate. **`useDashboardData`** runs before trend bounds so **`activeProgram.startDate`** can anchor the window. **Preset trend lengths (1–4 wk)** span **forward** from `startDate` (fallback today), not backward from today; charts and the selected day’s workout use **`useWorkouts(userId, { from, to })`** over that inclusive range. For the **selected calendar date** (`selectedTrendDate`), the page **also** composes **`useNutritionDay`**, **`useRecoveryEntry`**, and **`useSupplementLog`** so schedule + cards track the strip. Program **Week 1 start** is edited with **`useUpdateProgram`** (`ProgramCycleStartControl`). Do not fold arbitrary per-date UI into the bundle query key unless product requires a single cache entry — prefer these existing per-day keys so mutations invalidate the right documents.
